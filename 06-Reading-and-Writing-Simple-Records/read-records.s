...
_start:
	...
	# and the output descriptors (FYI, we could have used memory addresses
	# in a .data section instead).
	...
	movl $SYS_OPEN, %eax
	movl $file_name, %ebx
	movl $0, %ecx		# This say to open read-only
	movl $0666, %edx
	int $LINUX_SYSCALL

	# Sayve file descriptors

	movl %eax, ST_INPUT_DESCRIPTOR(%ebp)

# Even though it's a constant, we are saving the output file descriptor in
# a local variabl so that if we later decide that it isn't always going to
