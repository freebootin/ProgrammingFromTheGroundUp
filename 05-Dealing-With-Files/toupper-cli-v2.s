...
#			b) go through each byte of memory if the byte
...
syscall_error:
	# write to STDOUT
	movl $STDOUT, %ebx                         # file discriptor
        movl $err_msg, %ecx                         # buffer start
        movl $err_len, %edx                         # buffer size
        movl $SYS_WRITE, %eax
        int $LINUX_SYSCALL
...
#OUTPUT: This function overwrites the current buffer with the upper-cased version.
#
